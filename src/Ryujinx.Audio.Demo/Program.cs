...
using Ryujinx.HLE.Audio;

namespace Ryujinx.Audio.Demo
{
        ...
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                ...
                if (keyInfo.Key == ConsoleKey.Spacebar)
                {
                    ...
                    using (FileStream stream = new FileStream("长风破浪会有时，直挂云帆济沧海.wav", FileMode.Open))
                    {
                        outSystem.Update();
        ...
        static void PlayRowData(AudioOutputSystem outSystem, byte[] source, int offset, int count, WaveFormat format)
        {
            ...
            else
            {
                ...
                while (true)
                {
                    ...
                    {
                        DataPointer = 1,
                        Data = buffer.Select(p=>(byte)p).ToArray(),
